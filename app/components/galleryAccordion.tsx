...
const GallaryAccordion = () => {
  ...
  const handleImageClick = (imageArray, index) => {
    const fullImageUrls = imageArray.map((img) => `${process.env.NEXT_PUBLIC_Image_Path}/gallery_image/${img}`);
            ...
            <div key={index} className="py-4">
                ...
                <span className="flex gap-5 items-center text-lg font-medium text-black relative">
                  <div className="flex gap-5 items-center">
                    <div className={`px-2 py-2 min-w-fit text-sm ${bgColor}`}>
                      ...
                      <h3 className="sm:text-xl font-semibold">2025</h3>
                    </div>
                  </div>
                  {gallerydata.title}
                </span>
              ...
              {openIndex === index && (
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-5 pt-5">
                  {gallerydata.gallery.map((src, srcIndex) => (
                    <div
                      key={srcIndex}
